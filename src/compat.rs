...
///
/// This `enum` may be convenient to get the features supported by a specific Linux kernel,
/// without relying on the kernel version (which may not be accessible or patched).
/// For example, each variant can be used with [`Access::from_all()`]
/// to get the access rights defined by a specific version.
///
/// A typical scenario is when developers test their applications
/// with a specific set of supported Landlock features on their workstation.
/// It would be hazardous to rely on the *relative* maximum set of features
/// (e.g., `BitFlags::<AccessFs>::all()` or `BitFlags::ALL`,
/// which should not exist for non-exhaustive structs anyway)
/// because it could change insidiously over time with a minor crate update
...
#[non_exhaustive]
pub enum ABI {
    /// Kernel not supporting Landlock, either because it is not build with Landlock
...
///
/// This enables to guarantee consistent behaviors across crate users
