...
impl Access for AccessNet {
    // Roughly read (i.e. not all network actions are handled).
    fn from_read(abi: ABI) -> BitFlags<Self> {
        match abi {
            ABI::Unsupported | ABI::V1 | ABI::V2 | ABI::V3 => BitFlags::EMPTY,
            ABI::V4 => AccessNet::BindTcp.into(),
        }
    }

    // Roughly write (i.e. not all network actions are handled).
    fn from_write(abi: ABI) -> BitFlags<Self> {
        match abi {
            ABI::Unsupported | ABI::V1 | ABI::V2 | ABI::V3 => BitFlags::EMPTY,
            ABI::V4 => AccessNet::ConnectTcp.into(),
        }
    }
}

#[test]
fn consistent_access_net_rw() {
    for abi in ABI::iter() {
        let access_all = AccessNet::from_all(abi);
        let access_read = AccessNet::from_read(abi);
        let access_write = AccessNet::from_write(abi);
